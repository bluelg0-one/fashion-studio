...
JSON 형식만 답해줘. 다른 텍스트 없이.`
      ...
      // 누끼가 있으면 누끼 이미지에 필터 적용, 없으면 원본에 적용
      ...
      setFilterValues(prev => ({
        ...prev,
        [imgId]: { brightness: parsed.brightness, contrast: parsed.contrast, saturation: parsed.saturation }
      }))

      setImages(prev => prev.map(i =>
        i.id === imgId ? { ...i, editedUrl, status: 'done', aiLog: parsed.log } : i
      ))
    } catch (err) {
      setImages(prev => prev.map(i =>
        i.id === imgId ? { ...i, status: 'error', aiLog: `오류: ${err.message}` } : i
      ))
    } finally {
  ...
  const handleRemoveBg = useCallback(async (imgId) => {
    ...
    try {
      ...
      // 선택한 배경색 적용
      const finalUrl = await applyBackgroundColor(bgRemovedUrl, bgColor)

      setImages(prev => prev.map(i =>
        i.id === imgId
          ? { ...i, bgRemovedUrl: finalUrl, editedUrl: finalUrl, hasBgRemoved: true, aiLog: '배경이 깔끔하게 제거됐습니다. ✂️' }
          : i
      ))
    } catch (err) {
  ...
  // 배경색 적용 함수
  async function applyBackgroundColor(dataUrl, color) {
    return new Promise((resolve) => {
      const img = new Image()
      img.onload = () => {
  ...
  // 배경색 변경
  const handleBgColorChange = useCallback(async (imgId, color) => {
    setBgColor(color)
    const img = images.find(i => i.id === imgId)
    if (!img || !img.bgRemovedUrl) return

    // 투명 배경 상태에서 새 색상 적용
    const pureTransparentUrl = await removeBackground(img.file).catch(() => img.bgRemovedUrl)
    const finalUrl = await applyBackgroundColor(pureTransparentUrl, color)
    setImages(prev => prev.map(i =>
      i.id === imgId ? { ...i, editedUrl: finalUrl } : i
    ))
  }, [images, setImages])
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div
          ...
          style={{
            ...
            background: dragOver ? '#e8e8ff' : '#fff', transition: 'all 0.2s',
          }}
        >
          <div style={{ fontSize: 32, marginBottom: 8 }}>📁</div>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>사진 업로드</div>
          <div style={{ fontSize: 12, color: '#888', lineHeight: 1.5 }}>
            클릭하거나 여기로 드래그<br />여러 장 한번에 선택 가능
          </div>
              ...
              {images.map(img => (
                <div key={img.id} onClick={() => setSelectedId(img.id)}
                  ...
                  <div style={{ position: 'relative' }}>
                    <img src={img.editedUrl} alt="" style={{
                      width: 44, height: 44, objectFit: 'cover', borderRadius: 6, border: '1px solid #eee',
                      background: '#f8f8f8'
                    }} />
                    {img.hasBgRemoved && (
                      <div style={{
                  ...
                  <div style={{ flex: 1, overflow: 'hidden' }}>
                    ...
                    <div style={{ fontSize: 10, color: statusColor[img.status], marginTop: 2 }}>
                      ...
                      {img.hasBgRemoved && <span style={{ color: '#10b981', marginLeft: 4 }}>✂️누끼완료</span>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer' }}>
                      <input type="checkbox" checked={img.approved}
                        onChange={(e) => { e.stopPropagation(); setImages(prev => prev.map(i => i.id === img.id ? { ...i, approved: e.target.checked, needsWork: false } : i)) }}
                        style={{ accentColor: '#10b981', width: 14, height: 14 }} />
                      <span style={{ fontSize: 9, color: '#10b981' }}>확정</span>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer' }}>
                      <input type="checkbox" checked={img.needsWork}
                        onChange={(e) => { e.stopPropagation(); setImages(prev => prev.map(i => i.id === img.id ? { ...i, needsWork: e.target.checked, approved: false } : i)) }}
        ...
        {selected ? (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[
                ...
                { label: selected.hasBgRemoved ? 'AI 보정본 (누끼완료)' : 'AI 보정본', url: selected.editedUrl, tag: 'AFTER', tagColor: '#10b981' },
              ].map(item => (
                <div key={item.tag} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e8e8', overflow: 'hidden' }}>
                  <div style={{ padding: '10px 14px', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: item.tagColor, background: item.tagColor + '15', padding: '2px 8px', borderRadius: 20 }}>
                      {item.tag}
                    </span>
                  </div>
                  <div style={{ background: item.tag === 'AFTER' && selected.hasBgRemoved ? '#f0f0f0' : '#f8f8f8', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '2px solid #10b981', padding: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: '#166534', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {selected.hasBgRemoved && (
                  <span style={{ fontSize: 10, background: '#10b981', color: '#fff', padding: '2px 8px', borderRadius: 20 }}>완료</span>
                )}
              </div>
              <div style={{ fontSize: 11, color: '#888', marginBottom: 12 }}>
                Remove.bg AI가 배경을 자동으로 제거하고 원하는 배경색으로 바꿔줍니다
              </div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  {[
                    ...
                    { color: '#f5f5f5', label: '연회색' },
                    { color: '#f0ede8', label: '크림' },
                    { color: '#e8f0e8', label: '연두' },
                    ...
                    <div key={color} onClick={() => setBgColor(color)}
                      ...
                      <div style={{
                        width: 32, height: 32, borderRadius: 6, border: bgColor === color ? '2px solid #1a1a2e' : '1px solid #ddd',
                  ...
                  <div>
                    <input type="color" value={bgColor === 'transparent' ? '#ffffff' : bgColor}
                      ...
                      style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #ddd', cursor: 'pointer', padding: 2 }} />
                    <div style={{ fontSize: 9, color: '#888', textAlign: 'center', marginTop: 3 }}>직접</div>
                  </div>
                </div>
              </div>

              <button onClick={() => handleRemoveBg(selected.id)}
                disabled={removingBg[selected.id]}
                style={{
                  width: '100%', padding: '11px 0', borderRadius: 8, border: 'none',
                  background: removingBg[selected.id] ? '#ccc' : '#10b981',
              ...
              {selected.hasBgRemoved && (
                <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                  {[
                    { color: '#ffffff', label: '⬜ 흰배경' },
                    { color: '#f5f5f5', label: '🔲 회색' },
                    { color: '#f0ede8', label: '🟫 크림' },
                    { color: 'transparent', label: '🔳 투명' },
                  ].map(({ color, label }) => (
                    <button key={color}
                      onClick={async () => {
                        ...
                        if (!img) return
                        const finalUrl = await applyBackgroundColor(img.bgRemovedUrl, color === 'transparent' ? 'rgba(0,0,0,0)' : color)
                      ...
                      style={{
                        ...
                        background: '#f9f9f9', color: '#555', fontSize: 10, cursor: 'pointer',
                      }}>
                      {label}
                    </button>
                  ))}
            ...
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8e8e8', padding: 16 }}>
              ...
              {[
                ...
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  ...
                  <input type="range" min={min} max={max} value={getFilter(selected.id, key)}
              ...
              {selected.needsWork && (
                <div>
                  ...
                  <textarea value={refinePrompt} onChange={(e) => setRefinePrompt(e.target.value)}
                    placeholder="예: 배경을 더 밝게 해줘, 색상을 더 선명하게 해줘"
                  ...
                  <button
                    onClick={() => { if (!refinePrompt.trim()) return; runAiRefine(selected.id, refinePrompt); setRefinePrompt('') }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <button onClick={() => setImages(prev => prev.map(i => i.id === selected.id ? { ...i, approved: true, needsWork: false } : i))}
                style={{
                  ...
                  fontSize: 13, fontWeight: selected.approved ? 700 : 400,
                }}>
                ✅ 최종 확정
              </button>
              <button onClick={() => setImages(prev => prev.map(i => i.id === selected.id ? { ...i, needsWork: true, approved: false } : i))}
                style={{
                  ...
                  fontSize: 13, fontWeight: selected.needsWork ? 700 : 400,
                }}>
                🔧 추가 수정 필요
              </button>
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, border: '2px dashed #e8e8e8', height: 400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#aaa' }}>
            ...
            <div style={{ fontSize: 13, textAlign: 'center', lineHeight: 1.6 }}>
              왼쪽 업로드 영역에서 사진을 올린 후<br />AI 자동보정 또는 누끼 따기를 해보세요
            </div>
          </div>
        )}
      </div>
